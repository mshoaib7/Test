...
queryInstalled() {
    ...
    cat log1.txt
    PACKAGE_ID=$(sed -n "/${CC_NAME}_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log1.txt)
    echo PackageID is ${PACKAGE_ID}
    ...
    cat log2.txt
    PACKAGE_ID=$(sed -n "/${CC_NAME}_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log2.txt)
    echo PackageID is ${PACKAGE_ID}
...
approveForMyOrg1() {
    ...
    peer lifecycle chaincode approveformyorg -o localhost:7050 --tls \
        --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${VERSION} --init-required \
...
checkCommitReadyness() {
    ...
    #     --sequence ${VERSION} --output json --init-required --signature-policy "OR ('Org1MSP.peer','Org2MSP.peer')"

    peer lifecycle chaincode checkcommitreadiness \
        -o localhost:7050 --channelID $CHANNEL_NAME --tls \
        --cafile $ORDERER_CA --name ${CC_NAME} --version ${VERSION} \
        --init-required --sequence ${VERSION} --signature-policy "OR ('Org1MSP.peer','Org2MSP.peer')"
...
checkCommitReadyness() {
    ...
    #     --collections-config $PRIVATE_DATA_CONFIG \
    #     --name ${CC_NAME} --version ${VERSION} --sequence ${VERSION} --output json --init-required --signature-policy "OR ('Org1MSP.peer','Org2MSP.peer')"
...
commitChaincodeDefination() {
    ...
    #     --version ${VERSION} --sequence ${VERSION} --init-required --signature-policy "OR ('Org1MSP.peer','Org2MSP.peer')"

    peer lifecycle chaincode commit -o localhost:7050 --channelID \
        ${CHANNEL_NAME} --name ${CC_NAME} --version ${VERSION} --sequence ${VERSION} \
        --init-required --tls --cafile $ORDERER_CA \
        --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_ORG1_CA  \
        --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_ORG2_CA  \
        --signature-policy "OR ('Org1MSP.peer','Org2MSP.peer')"

    echo "===================== checking commit chaincode defination ======================= "
}
